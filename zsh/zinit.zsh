...
if [[ ! -f $HOME/.zi/bin/zinit.zsh ]]; then
  print -P "%F{33}▓▒░ %F{160}Installing (%F{33}z-shell/zi%F{160})…%f"
  command mkdir -p "$HOME/.zi" && command chmod g-rwX "$HOME/.zi"
  command git clone -q --depth=1 --branch "main" https://github.com/zdharma-continuum/zinit "$HOME/.zi/bin" && \
  print -P "%F{33}▓▒░ %F{34}Installation successful.%f%b" || \
  print -P "%F{160}▓▒░ The clone has failed.%f%b"
fi

source "$HOME/.zi/bin/zinit.zsh"
autoload -Uz _zi
...
zinit wait lucid for \
 atinit"ZINIT[COMPINIT_OPTS]=-C; zpcompinit; zicompinit; zicdreplay" \
...
# Optionally load other plugins here (autosuggestions, history search, etc.)
zinit load zsh-users/zsh-autosuggestions

# Load fast-syntax-highlighting LAST
zinit load zdharma-continuum/fast-syntax-highlighting
zinit load z-shell/fast-syntax-highlighting
